...
class PublicView(unittest.TestCase):
    ...
    def test_it_counts_how_the_answers_arrived(self):
        ...
        self.assertEqual(pub["answers_7d"], {"voice": 2, "tap": 2})                          # the unanswered day is not counted

    def test_only_the_last_seven_days_are_counted(self):
        doses = days_of(*["on_time"] * 10)
        for x in doses[:3]:
            x.via = "voice"
        self.assertEqual(a.answers_by(summary(doses, ts(9, 12))), {"voice": 0, "tap": 7})
        self.assertEqual(a.answers_by(summary(doses, ts(9, 12)), days=10), {"voice": 3, "tap": 7})

    def test_a_dose_still_waiting_is_not_an_answer(self):
        self.assertEqual(a.answers_by(summary([dose(0, "none")], ts(0, 8, 30))), {"voice": 0, "tap": 0})
